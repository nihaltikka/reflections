...
def print_banner():
    ...
    {GREEN}#        {BOLD}Developed by elcazad0r{RESET}{GREEN}        #
    {GREEN}#                                       #
    {GREEN}#########################################{RESET}
    """
    print(banner)
...
def save_reflected_url(url, modified_path, output_file):
    """Save reflected URLs with '{payload}' replacing 'elcazad0rXSS'"""
    payload_url = urlunparse(url._replace(path=modified_path)).replace("elcazad0rXSS", "{payload}")
...
def check_reflection(url, output_file):
    ...
    try:
        ...
        for i, segment in enumerate(path_segments):
            ...
            # Inject 'elcazad0rXSS' only for supported extensions
            if '.' in segment:
                base, ext = segment.rsplit('.', 1)
                if ext in SUPPORTED_EXTENSIONS:
                    modified_segments[i] = f'elcazad0rXSS.{ext}'
            else:
                # For non-extension segments, inject 'elcazad0rXSS'
                modified_segments[i] = 'elcazad0rXSS'
            ...
            if 'elcazad0rXSS' in response.text:
                print(f"{GREEN}[+] Reflection found: {modified_url}{RESET}")
                save_reflected_url(parsed_url, modified_path, output_file)

        # Test appending 'elcazad0rXSS' at the end of the path
        appended_path = parsed_url.path.rstrip('/') + '/elcazad0rXSS'
...
def post_process_urls(input_file, output_file):
    """Post-process URLs to remove duplicates and replace 'elcazad0rXSS' with '{payload}'"""
    print(f"{CYAN}Processing URLs to replace 'elcazad0rXSS' with '{{payload}}'...{RESET}")
    temp_file = f"{output_file}.tmp"
    with open(input_file, 'r') as infile, open(temp_file, 'w') as outfile:
        urls = set()
        for line in infile:
            line = line.strip().replace("elcazad0rXSS", "{payload}")
            urls.add(line)
